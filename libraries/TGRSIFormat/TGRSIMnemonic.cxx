...
double TGRSIMnemonic::GetTime(Long64_t timestamp, Float_t cfd, TChannel* channel)
{
   if(channel == nullptr) {
   ...
   switch(static_cast<EDigitizer>(channel->GetDigitizerType())) {
		...
		case EDigitizer::kGRF16:
			...
			dTime = (timestamp & (~0x3ffff)) * channel->GetTimeStampUnit() + channel->CalibrateCFD((GetCfd() + gRandom->Uniform()) / 1.6); // CFD is in 10/16th of a nanosecond
			return SetTime(dTime - channel->GetTZero(GetEnergy()) - channel->GetTimeOffset());
		case EDigitizer::kGRF4G:
			dTime = timestamp*channel->GetTimeStampUnit() + channel->CalibrateCFD((static_cast<Int_t>(fCfd) >> 22) + ((static_cast<Int_t>(fCfd) & 0x3fffff) + gRandom->Uniform()) / 256.);
			return SetTime(dTime - channel->GetTZero(GetEnergy()) - channel->GetTimeOffset());
		case EDigitizer::kTIG10:
			dTime = (timestamp & (~0x7fffff))*channel->GetTimeStampUnit() +	channel->CalibrateCFD((GetCfd() + gRandom->Uniform()) / 1.6); // CFD is in 10/16th of a nanosecond
      ...
      case EDigitizer::kCaen:
			...
			dTime = timestamp*channel->GetTimeStampUnit() + channel->CalibrateCFD((GetCfd() + gRandom->Uniform()) / 512.);
			return SetTime(dTime - channel->GetTZero(GetEnergy()) - channel->GetTimeOffset());
		default:
