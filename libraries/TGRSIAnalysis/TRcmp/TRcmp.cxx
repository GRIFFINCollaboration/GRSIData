...
void TRcmp::AddFragment(const std::shared_ptr<const TFragment>& frag, TChannel* chan)
{
   ...
   else if(mnemonic->CollectedCharge() == TMnemonic::EMnemonic::kN) {
      fBackNFragVector.push_back(*frag);
   }   // add the fragment to the back vector is collected charge is on the N-side
}
